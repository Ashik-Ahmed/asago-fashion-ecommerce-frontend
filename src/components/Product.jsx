...
const Product = ({ product, bg }) => {
    ...
    return (
        <div className="w-full relative group border-[1px] border-black hover:shadow-lg duration-200 shadow-gray-500 rounded-md overflow-hidden group">
            <div className="w-full h-80 flex items-center justify-center bg-white overflow-hidden relative">
                <div className={`w-full h-full relative ${bg}`}>
                    ...
                    <div className="absolute bottom-0 flex items-center gap-5 justify-around w-full translate-y-[110%] group-hover:-translate-y-2 transition-transform duration-300">
                        <button
                            onClick={() => { dispatch(addToCart(product)); toast.success(`${product?.title.substring(0, 12)}... added to cart`) }}
                            className="bg-gray-800 text-gray-200 px-4 py-2 text-xs rounded-full flex items-center gap-1 hover:bg-sky-400 hover:text-white duration-200"
                        >
                            <span>
                                <AiOutlineShopping />
                            </span>
